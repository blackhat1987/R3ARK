...
BEGIN_MESSAGE_MAP(CMainCDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB_MAIN, &CMainCDlg::OnSelchangeTabMain)
END_MESSAGE_MAP()
...
BOOL CMainCDlg::OnInitDialog()
{
	...
	// TODO: 在此添加额外的初始化代码
	m_cfgTab = mfc::TabCtrl(&m_tabMain);
	m_cfgTab.insertPage(IDD_DIALOG_INI, L"INI操作", &m_iniDlg);
	m_cfgTab.insertPage(IDD_DIALOG_PROCESS, L"进程", &m_processDlg);
...
void CMainCDlg::OnSelchangeTabMain(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	m_cfgTab.tabChange();
	*pResult = 0;
